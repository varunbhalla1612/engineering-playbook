...
Use a hash map to store previously seen numbers and their indices. For each element, compute its complement (target - current). If the complement already exists in the map, return the indices; otherwise, store the current element and continue.
